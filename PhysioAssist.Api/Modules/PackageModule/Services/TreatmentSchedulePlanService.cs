        ...
        IUnitOfWork _unitOfWork) : ITreatmentSchedulePlanService
{
        ...
        Guid reportId, UpsertTreatmentSchedulePlanRequest request, CancellationToken cancellationToken = default)
    {
        ...
        else if (plan.LifeCycleStatus != TreatmentSchedulePlanStatus.Pending)
        {
    ...
    {
        // unchanged — CreatePackageWithFirstBookingAsync still lives on IPatientSessionSchedulingService
        var report = await _reportRepository.GetByIdAsync(reportId);
        if (report is null)
            return Result.Failure<TreatmentSchedulePlanResponse>(TreatmentSchedulePlanErrors.NotFound);
        ...
        if (plan.LifeCycleStatus != TreatmentSchedulePlanStatus.Pending)
            return Result.Failure<TreatmentSchedulePlanResponse>(TreatmentSchedulePlanErrors.AlreadyResolved);
        ...
        var packageResult = await _PatientSessionSchedulingService.CreatePackageWithFirstBookingAsync(new CreatePackageWithFirstBookingRequest
        {
            ...
            TreatmentSchedulePlanId = plan.Id,
        ...
        plan.LifeCycleStatus = TreatmentSchedulePlanStatus.Booked;
    ...
    public async Task<Result<TreatmentSchedulePlanResponse>> SendToReceptionistAsync(Guid reportId, CancellationToken cancellationToken = default)
    {
        ...
        plan.LifeCycleStatus = TreatmentSchedulePlanStatus.SentToReceptionist;
    ...
    {
        // unchanged — GetTopRecommendedSlotsAsync stays on IPatientSessionSchedulingService
        if (plan.LifeCycleStatus != TreatmentSchedulePlanStatus.Pending)
            return [];
        ...
        Guid patientId, CancellationToken cancellationToken = default)
    {
        ...
        if (plan is null || plan.LifeCycleStatus == TreatmentSchedulePlanStatus.Pending)
            return Result.Success(new PatientSchedulingContextDto { State = PatientSchedulingState.PlanPending });

        if (plan.LifeCycleStatus == TreatmentSchedulePlanStatus.SentToReceptionist)
        {
        ...
        // FIX: GetPackageSummaryByPlanIdAsync now lives on _packageService, not _PatientSessionSchedulingService
        var summaryResult = await _packageService.GetPackageSummaryByPlanIdAsync(plan.Id, cancellationToken);
        ...
        Guid treatmentPlanId, ConvertPlanToPackageRequest request, CancellationToken cancellationToken = default)
    {
        ...
        // FIX: CreatePackageAsync now lives on _packageService, and FirstSessionSlot no longer exists on the DTO
        ...
        // FIX: GetPackageSummaryAsync now lives on _packageService
