...
export class SubmissionDetailComponent implements OnInit {
  ...
  @ViewChild(SubmittedAnswersViewerComponent)
  private answersViewer?: SubmittedAnswersViewerComponent;

  readonly chiefComplaintDisplay = computed(() => {
    const questionId =
      this.findQuestionIdByText('Chief Complaint') ?? 'question_default_chief_complaint';
  ...
  readonly patientNameDisplay = computed(() => {
    const questionId =
      this.findQuestionIdByText('Full Name') ??
      this.findQuestionIdByText('Name') ??
      'question_default_full_name';
    return this.extractAnswer(questionId);
  });
  readonly patientEmailDisplay = computed(() => {
    const questionId =
      this.findQuestionIdByText('Email') ??
      this.findQuestionIdByText('E-mail') ??
      'question_default_email';
    return this.extractAnswer(questionId);
  });
  readonly patientPhoneDisplay = computed(() => {
    const questionId =
      this.findQuestionIdByText('Phone') ??
      this.findQuestionIdByText('Phone Number') ??
  ...
  readonly canEdit = computed(() => {
    ...
    const statusOk =
      status != null && status !== IntakeStatus.Converted && status !== IntakeStatus.Expired;
    ...
    this.intakeApi
      ...
      .subscribe({
        next: (data) => {
          ...
          this.intakeApi
            .getFormSchemaById(data.formSchemaId)
            .pipe(takeUntilDestroyed(this.destroyRef))
            .subscribe({
              next: (schemaResponse) => {
                try {
  ...
  attemptConvert(): void {
    if (!this.canEditPermission()) {
      this.snackbar.error('Not permitted', [
        'You do not have permission to convert this submission.',
      ]);
      return;
    }
  ...
  updateStatus(newStatus: IntakeStatus): void {
    ...
    this.intakeApi
      .updateIntakeStatus(id, request)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: () => {
          this.snackbar.success('Status Updated', [
            `Submission moved to ${this.getStatusLabel(newStatus)}.`,
          ]);
        ...
        error: (err: any) => {
          this.updating.set(false);
          const msg =
            err?.error?.detail || err?.error?.title || 'Could not update submission status.';
  ...
  convertToPatient(): void {
    ...
    const request: ConvertIntakeToPatientRequest = {
      formSubmissionData: submission
        ...
        : (this.details()?.formSubmissionData ?? '{}'),
      painPointsData:
        painMap && painMap.regions && painMap.regions.length > 0
          ? JSON.stringify(painMap)
          : (this.details()?.painPointsData ?? undefined),
    };

    this.intakeApi
      .convertToPatient(id, request)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: (res: PreVisitIntakeResponse) => {
          this.snackbar.success('Conversion Successful', [
            'Submission has been converted to a patient record.',
          ]);
          this.updating.set(false);
        ...
        error: (err: any) => {
          ...
          const msg =
            ...
            err?.error?.title ||
            err?.error?.message ||
            'Could not convert submission to patient.';
